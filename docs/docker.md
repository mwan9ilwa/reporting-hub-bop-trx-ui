...
```bash
docker build -t reporting-hub-bop-trx-ui-internal:local .
```
...
```bash
docker run -p 8081:8081 reporting-hub-bop-trx-ui-internal:local
```
...
docker build \
  ...
  -t reporting-hub-bop-trx-ui-internal \
  .
```
