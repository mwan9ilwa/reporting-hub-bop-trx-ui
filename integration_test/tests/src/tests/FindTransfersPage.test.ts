...
fixture`Find Transfers Feature`.page`${config.transfersMicrofrontendEndpoint}`
  .before(async (ctx) => {
    const cli = new VoodooClient('ws://localhost:3030/voodoo', {
      defaultTimeout: config.voodooTimeoutMs,
    });
    await cli.connected();
...
test.meta({
  ID: '',
  STORY: '',
})('Navigate to Technical Details Tab of Transfer Modal popup', async (t) => {
  // navigate to the find transfers page
  await t.click(SideMenu.transfersButton).wait(1000);
...
test.meta({
  ...
  await t
    .typeText(FindTransfersPage.transferIdSearchField, t.fixtureCtx.transfers[0].transfer_id)
    .wait(1000);
  ...
  await t
    .expect(rowIds[0])
    .eql(t.fixtureCtx.transfers[0].transfer_id, 'rows dont contain transfer');
});
...
test.only.meta({
  ...
  await t
    .expect(FindTransfersPage.getChart('TransfersByCurrencyChart').exists)
    .ok('TransfersByCurrencyChart not found');
  await t
    .expect(FindTransfersPage.getChart('TransfersByPayerChart').exists)
    .ok('TransfersByPayerChart not found');
  await t
    .expect(FindTransfersPage.getChart('TransfersByPayeeChart').exists)
    .ok('TransfersByPayeeChart not found');
  await t
    .expect(FindTransfersPage.getChart('ErrorsByErrorCodeChart').exists)
    .ok('ErrorsByErrorCodeChart not found');
  await t
    .expect(FindTransfersPage.getChart('ErrorsByPayerChart').exists)
    .ok('ErrorsByPayerChart not found');
  await t
    .expect(FindTransfersPage.getChart('ErrorsByPayeeChart').exists)
    .ok('ErrorsByPayeeChart not found');
});

test.skip.meta({
  ID: '',
  STORY: '',
})('Filter Transfers by Payer FSPID, Payer Id and Payer value', async (t) => {
...
test.skip.meta({
  ID: '',
  STORY: '',
})('Filter Transfers by Payee FSPID, Payee Id and Payee value', async (t) => {
  // To be able to perform filters on Payer/Payee we need to
  // perform quotes with the participants so that they populate the
  // `quoteParty` table. This will involve having to rework the harness
  // to use the testing toolkit.
...
test.skip.meta({
  ID: '',
  STORY: '',
})('Navigate to Party Events Modal of Transfer Modal', async (t) => {
...
test.skip.meta({
  ID: '',
  STORY: '',
})('Navigate to Quote Events Modal of Transfer Modal', async (t) => {
...
test.skip.meta({
  ID: '',
  STORY: '',
})('Navigate to Transfer Events Modal of Transfer Modal', async (t) => {
  // This requires upgrading the test harness to initiate party lookup
...
test.skip.meta({
  ID: '',
  STORY: '',
})('Navigate to Settlement Events Modal of Transfer Modal', async (t) => {
